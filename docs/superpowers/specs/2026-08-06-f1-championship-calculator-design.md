...
3. **Probability method (AMENDED during implementation, 2026-08-06):** Monte Carlo simulation (3,000+ runs). Originally specced as a normal distribution fitted to each driver's points-per-race mean/stddev; **changed to empirical resampling** (each simulated race draws a random value from the driver's own actual 2026 per-race points history) after code review during Task 3 found the normal-distribution approach systematically *understated* high-pace leaders — for a driver whose average sits close to the 25-point cap (e.g. the real 2026 leader, avg≈18/stddev≈9.8), the cap is much closer to the mean than the floor is, so any bell-curve fit (clamped or truncated) pulls the effective simulated mean down, in exactly the wrong direction for the feature's stated goal. Empirical resampling has no distributional-shape assumption to get wrong: it can only ever produce point values the driver has actually scored, so it's naturally bounded and captures real skew. `computePaceStats` (avgGP/stdGP/avgSprint/stdSprint) is still computed and kept on each driver for *display* purposes (e.g. a future "pace" stat), but the simulation itself samples from the raw per-race history arrays, not from the fitted distribution.
